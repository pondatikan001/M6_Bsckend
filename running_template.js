    ...
    switch (request_path.pathname) {
        case '/fill':
            try {
                ...
                console.log(`Add stock name = ${request_path.query.item} จำนวน = ${request_path.query.quantity}`) 
            } catch (err) {
                status = 400;
                message += err
                console.log(err)
        ...
        case '/sell':
            try {
                ...
                console.log(`Sell stock name = ${request_path.query.item} จำนวน = ${request_path.query.quantity}`)
            } catch (err) {
                status = 400;
                message += err
                console.log(err)
        ...
        case '/check':
            try {
                ...
                console.log(`Have stock ${request_path.query.item} ใน stock จำนวน = ${qtt}`) 
            } catch (err) {
                status = 400;
                message += err
                console.log(err)
